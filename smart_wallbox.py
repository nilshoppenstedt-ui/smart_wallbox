...
LESER_IP = "192.168.178.191"

def get_power():
    """Return current grid power in W (positive=Bezug, negativ=Einspeisung)."""
    url = f"http://{LESER_IP}/cm?cmnd=Status%2010"
    r = requests.get(url, timeout=3)
    data = r.json()
    sns = data.get("StatusSNS", {})
    mt  = sns.get("MT631", {})
    return mt.get("Power_cur")

if __name__ == "__main__":
    while True:
        p = get_power()
        if p is not None:
            print(f"Aktuelle Leistung: {p} W")
        else:
            print("Keine Leistungsdaten erhalten.")
        time.sleep(2)
...
def read_wb_power_kw(ip):
    """Read go-e POWER_TOTAL via Modbus and return kW."""
    client = ModbusTcpClient(ip, port=502, timeout=3)
    client.connect()
    try:
        # POWER_TOTAL: Input Register, Adresse 120, Länge 2
        rr = client.read_input_registers(120, count=2, device_id=1)

        if rr.isError():
            raise RuntimeError(f"Error reading POWER_TOTAL: {rr}")

        regs = rr.registers  # [low_word, high_word]
        print("POWER_TOTAL raw registers:", regs)

        # Register 120 = High-Word, Register 121 = Low-Word
        raw32 = (regs[0] << 16) | regs[1]

        # Einheit: 0.01 W → 100000 = 1 kW
        power_kw = raw32 / 100000.0
        return power_kw
    finally:
        client.close()


raw = read_wb_power_kw("192.168.178.21")
print(raw, "kW")
...
def read_wb_power_kw(ip):
    ...
    regs = rr.registers  # list of 2 uint16
    raw = (regs[0] << 16) | regs[1]   # combine into uint32

    client.close()

    return raw / 100000.0  # kW
...
def main():
    ...
    while True:
        try:
            if PV_SURPLUS_MODE:
                # Sample grid power every 10 seconds
                if counter % 10 == 0:
                    grid_kw = read_grid_power_kw()
                    grid_list.append(grid_kw)
                    # Debug:
                    # print(f"Grid power (instant): {grid_kw:.2f} kW")
                ...
                if grid_list and (counter == 299 or is_startup):
                    ...
                    # Available PV power for system: P_WB - P_grid
                    ...
                    # Debug:
                    print(f"Grid_avg: {grid_avg_kw:.2f} kW, "
                          f"WB_power: {wb_power_kw:.2f} kW, "
                          f"Available: {available_kw:.2f} kW, "
                          f"Effective: {effective_kw:.2f} kW")
